...
function repeatString(str, times) {
  if (times < 1) return '';
  if (str !== '') return str.repeat(times);
  return '';
}
...
function countVowels(str) {
  const glassnye = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (glassnye.includes(str[i])) count += 1;
  }
  return count;
}
...
function findLongestWord(sentence) {
  let dlina = 0;
  let slovo = '';
  const massiv = sentence.split(' ');
  for (let i = 0; i < massiv.length; i += 1) {
    if (massiv[i].length > dlina) {
      dlina = massiv[i].length;
      slovo = massiv[i];
    }
  }
  return slovo;
...
function invertCase(str) {
  let strr = str.split('');
  for (let i = 0; i < strr.length; i += 1) {
    if (strr[i] === strr[i].toLowerCase()) strr[i] = strr[i].toUpperCase();
    else strr[i] = strr[i].toLowerCase();
  }
  strr = strr.join('');
  return strr;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (x <= 90 && x >= 65)
      rez += String.fromCharCode(65 + ((x + 13 - 65) % 26));
    else if (x <= 122 && x >= 97)
      rez += String.fromCharCode(97 + ((x + 13 - 97) % 26));
    else rez += str[i];
  }
  return rez;
}
...
function getCardId(value) {
  ...
  for (let i = 0; i < karty.length; i += 1) {
    if (karty[i] === value) return i;
  }
  return 0;
}
